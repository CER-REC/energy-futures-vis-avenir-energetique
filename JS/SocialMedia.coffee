...
SocialMedia = (app) ->
  ...
  d3.select "#canadasEnergyFutureVisualization .twitterItem" 
    .on 'click', ->
      ShortLinkBitly (url) ->
        window.open "https://twitter.com/intent/tweet?url=#{url}", 'targetWindow', 'width=650,height=650'

  d3.select "#canadasEnergyFutureVisualization .linkedinItem"
    .on 'click', ->
      ShortLinkBitly (url) ->
        window.open "https://www.linkedin.com/shareArticle?mini=true&url=#{url}&summary=#{url}", 'targetWindow', 'width=650,height=650'
