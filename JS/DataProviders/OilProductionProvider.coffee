...
class OilProductionProvider
  ...
  dataForAllViz1Scenarios: (viz1config) ->
    ...
    return filteredProvinceData if viz1config.unit == 'thousandCubicMetres'

    if viz1config.unit == 'kilobarrels' 
      ...
      for province in Object.keys filteredProvinceData
        ...
        for item in filteredProvinceData[province]
          unitConvertedProvinceData[province].push 
            ...
            year: item.year
            value: item.value * UnitTransformation.transformUnits('thousandCubicMetres', 'kilobarrels')
  ...
  dataForAllViz4Scenarios: (viz4config) ->
    ...
    # Finally, convert units
    return filteredScenarioData if viz4config.unit == 'thousandCubicMetres'

    if viz4config.unit == 'kilobarrels' 
